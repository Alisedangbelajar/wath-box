...
export const StrapWorkshop: React.FC<StrapWorkshopProps> = ({
  ...
  return (
    <div className="p-4 space-y-4 text-slate-800">
      ...
      <div className="bg-slate-50 rounded-2xl border border-slate-200/90 p-4 shadow-2xs space-y-3">
        <div className="flex items-center justify-between">
          ...
          {isCompatible ? (
            <span className="text-[10px] font-mono-tech text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full flex items-center gap-1 font-semibold">
              <Check className="w-3 h-3" /> Compatible ({selectedStrap?.lugWidthMm}mm Lug)
            </span>
          ) : (
            <span className="text-[10px] font-mono-tech text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full flex items-center gap-1 font-semibold">
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-200/80 flex flex-col items-center justify-center relative overflow-hidden min-h-[190px] shadow-inner">
          {/* Top Strap Segment Graphic */}
          <div 
            className="w-16 h-12 rounded-t-lg bg-slate-700 border-x border-t border-slate-600 shadow-inner flex items-center justify-center relative"
            style={{
              backgroundColor: selectedStrap?.color.includes('Brown') || selectedStrap?.color.includes('Cognac') ? '#78350f' : '#334155',
            }}
          >
            <div className="w-full h-full bg-black/10" />
            <div className="absolute top-1 text-[8px] font-mono-tech text-white/70 font-semibold">{selectedStrap?.lugWidthMm}mm</div>
          </div>
          ...
          <div className="relative z-10 my-1">
            <img
              ...
              className="w-24 h-24 rounded-full object-cover border-2 border-slate-200 shadow-md ring-4 ring-white"
          ...
          <div 
            className="w-14 h-16 rounded-b-xl bg-slate-700 border-x border-b border-slate-600 shadow-inner flex flex-col items-center justify-between p-1"
            style={{
              backgroundColor: selectedStrap?.color.includes('Brown') || selectedStrap?.color.includes('Cognac') ? '#78350f' : '#334155',
            }}
          >
            <div className="w-1 h-3 rounded-full bg-black/30 mt-1" />
            <div className="w-7 h-3 rounded-xs border border-indigo-200 bg-white text-[7px] text-indigo-700 font-mono-tech font-bold text-center">
          ...
          <div className="mt-3 text-center">
            <h4 className="text-xs font-bold text-slate-800 font-serif-luxury">
              {selectedWatch?.brand} {selectedWatch?.nickname || selectedWatch?.model}
            </h4>
            <p className="text-[10px] text-indigo-600 font-mono-tech font-bold mt-0.5">
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="text-[11px] font-mono-tech text-slate-500">
            Current: <strong className="text-slate-800">{isCurrentlyMounted ? 'Mounted on Watch' : 'Available in Wardrobe'}</strong>
          </div>
      ...
      <div className="space-y-1.5">
        <label className="text-[10px] text-slate-500 font-mono-tech uppercase font-bold block">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl border shrink-0 transition-all text-xs ${
                  isSelected
                    ? 'bg-indigo-50 border-indigo-600 text-indigo-900 shadow-2xs font-semibold'
                    : 'bg-slate-50 border-slate-200 text-slate-600 hover:text-slate-900'
                }`}
              >
                <div>
                  <div className="text-[9px] uppercase font-mono-tech text-indigo-600 font-bold">{w.brand}</div>
                  <div className="truncate max-w-[100px] font-medium">{w.nickname || w.model}</div>
                  <div className="text-[9px] text-slate-400 font-mono-tech">{w.dimensions.lugWidthMm}mm Lug</div>
                </div>
              </button>
            );
          })}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[10px] font-mono-tech text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-md hover:bg-indigo-100 transition-colors font-semibold"
          >
        ...
        <div className="flex items-center gap-1.5 text-[11px]">
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 focus:outline-indigo-500 shadow-2xs"
          >
          ...
          return (
            <div
              ...
              className={`p-3 rounded-2xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${
                isSelected
                  ? 'bg-indigo-50 border-indigo-600 shadow-2xs ring-1 ring-indigo-600'
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] font-mono-tech text-indigo-600 font-bold uppercase">{strap.brand}</span>
                    {strap.waterproof && (
                      <span className="text-[9px] text-cyan-700 bg-cyan-50 px-1 rounded flex items-center gap-0.5 font-semibold">
                        <Droplets className="w-2.5 h-2.5" /> Waterproof
                      </span>
                    )}
                  </div>
                  <h4 className="font-semibold text-xs text-slate-800 mt-0.5">{strap.name}</h4>
              ...
              <div className="text-right shrink-0">
                <span className="font-mono-tech font-bold text-xs text-indigo-600 block">${strap.priceUsd}</span>
                {fitsCurrentWatch ? (
                  <span className="text-[9px] text-emerald-600 font-mono-tech font-semibold">Fits {selectedWatch?.brand}</span>
                ) : (
                  <span className="text-[9px] text-slate-400 font-mono-tech">{strap.lugWidthMm}mm</span>
                )}
              </div>
            </div>
          );
        ...
        <div className="fixed inset-0 z-60 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-5 w-full max-w-sm space-y-3 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h4 className="text-sm font-bold font-serif-luxury text-slate-800">Add Strap to Wardrobe</h4>
              <button onClick={() => setShowAddStrapModal(false)} className="text-slate-400 hover:text-slate-700 font-bold">
                ✕
              </button>
            </div>

            <form onSubmit={handleCreateStrap} className="space-y-2.5 text-xs text-slate-800">
              <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] text-slate-500 font-mono-tech font-bold block mb-1">Brand / Maker</label>
                  <input
                    type="text"
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-indigo-600 text-white font-bold rounded-xl font-mono-tech mt-2 hover:bg-indigo-500 shadow-md"
              >
                Add To Wardrobe
              </button>
            </form>
